...
import { createSupabaseForAccessToken, supabase } from './supabase.js';
...
export function isLocalDemoAuth(auth: AuthContext) {
  return auth.accessToken === LOCAL_DEMO_TOKEN;
}
...
export type AuthContext = {
  ...
  db: ReturnType<typeof createSupabaseForAccessToken>;
};
...
export async function getAuthContext(req: Request, res: Response): Promise<AuthContext | null> {
  ...
  return {
    ...
    db: createSupabaseForAccessToken(accessToken),
  };
}
